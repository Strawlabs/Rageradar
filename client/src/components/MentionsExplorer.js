...
const MentionsExplorer = () => {
  ...
  // Show empty state if no mentions match current filters
  if (filteredMentions.length === 0 && mentions.length > 0) {
    return (
      <div className="p-6 w-full">
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">No mentions match the current filters</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{mentions.length} mentions available — try adjusting your filters.</p>
          <button
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-4">
                    ...
                    {(mention.url || mention.link) && (
                      ...
                      >
                        ...
                        {new URL(mention.url || mention.link).hostname.replace('www.', '')}
                      </a>
                    )}
                  </div>
                  ...
                  <div className="flex items-center space-x-3 flex-wrap gap-y-1">
                    ...
                    {(mention.rageIndex > 0 || mention.rageScore > 0) && (() => {
                      ...
                      return (
                        <div className={`px-2.5 py-1 rounded-full text-xs font-bold ${riColor}`} title="Rage Index">
                          RI {ri}
                        </div>
                      );
                    })()}
                      ...
                      <div className="flex flex-wrap gap-1">
                        ...
                        {mention.keywords.length > 3 && (
                          <span className="text-xs text-muted-foreground">
                            +{mention.keywords.length - 3} more
                          </span>
                        )}
                      </div>
                    </div>
                    {/* Theme tags */}
                    {mention.themes && mention.themes.length > 0 && (
                      <div className="flex items-center space-x-2">
                        <span className="text-sm text-muted-foreground">Themes:</span>
                        <div className="flex flex-wrap gap-1">
                          {mention.themes.slice(0, 4).map((theme, index) => (
                            <span key={index} className="px-2 py-1 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 text-xs rounded-full border border-purple-200 dark:border-purple-800">
                              #{typeof theme === 'string' ? theme : (theme.theme || theme.name || theme.label || '')}
                            </span>
                          ))}
                        </div>
                      </div>
